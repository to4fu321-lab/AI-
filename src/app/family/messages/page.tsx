      ...
      <div className="flex-1 px-5 py-4 flex flex-col gap-4 overflow-y-auto pb-72">
        {messages.length === 0 ? (
          ...
          messages.map((msg) => (
            <div
              key={msg.id}
              className={`flex ${msg.from === 'family' ? 'justify-end' : 'justify-start'}`}
            >
              <div
                className={`max-w-xs rounded-3xl px-5 py-4 shadow-sm ${
                  msg.from === 'family'
        ...
        <div className="flex gap-2 mb-3 overflow-x-auto pb-1">
          {QUICK_MESSAGES.map((qm) => (
            <button
              ...
              className="flex-shrink-0 bg-amber-50 border border-amber-200 text-amber-800 rounded-2xl px-3 py-2 text-sm font-medium active:bg-amber-100 transition-colors whitespace-nowrap"
            >
              {qm}
            </button>
          ))}
